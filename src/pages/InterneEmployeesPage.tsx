...
function BulkMatriculeModal({
  ...
  const handleExcelPreview = async (file: File) => {
    ...
    try {
      ...
      if (changed > 0) {
        toast.success(
          `${changed} changement(s) détecté(s)${conflict > 0 ? ` · ${conflict} conflit(s)` : ""}${not_found > 0 ? ` · ${not_found} introuvable(s)` : ""}`
        );
      } else {
  ...
  const handleExcelFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) handleExcelPreview(file);
  };
    ...
    >
      ...
      >
          ...
          <div className="px-6 pt-4 shrink-0">
            <div className="rounded-xl bg-slate-50 border border-slate-200 px-4 py-3 flex flex-wrap gap-4 text-sm">
              <span className="flex items-center gap-1.5 text-emerald-700 font-medium">
                ...
                {previewSummary.changed} à modifier
              </span>
              ...
              {previewSummary.conflict > 0 && (
                <>
                  ...
                  <span className="flex items-center gap-1.5 text-red-600 font-medium">
                    ...
                    {previewSummary.conflict} conflit(s)
                  </span>
                </>
              )}
              {previewSummary.not_found > 0 && (
                <>
                  ...
                  <span className="flex items-center gap-1.5 text-amber-600 font-medium">
                    <FiAlertTriangle size={14} />
                    {previewSummary.not_found} introuvable(s)
                  </span>
                </>
              )}
            </div>
        ...
        {isDone && resultSummary && (
          <div
            className={`mx-6 mt-3 rounded-xl px-4 py-3 flex items-start gap-3 text-sm shrink-0 ${
            ...
            {(!isDone || (resultSummary && resultSummary.errors.length > 0)) && (
              ...
              >
                {isSubmitting ? (
                  ...
                  <>
                    <FiEdit3 size={14} />
                    Mettre à jour {changedCount > 0 ? `(${changedCount})` : ""}
                  </>
                )}
              </button>
            )}
  ...
  return (
    <AppLayout>
      ...
      >
        ...
        {/* Modal bulk mise à jour matricules */}
        <AnimatePresence>
          {bulkMatOpen && (
            <BulkMatriculeModal
              employees={employees}
        ...
        {/* Modal résultat import (auto-dismiss 5s) */}
        <AnimatePresence>
          {importResult && (
            <ImportResultModal
              result={importResult}
