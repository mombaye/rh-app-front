...
import {
  getEmployees, importEmployees, markExit, reinstate,
  bulkUpdateMatricules, previewMatriculeChanges,
  MatriculeUpdate, MatriculeChange,
} from "@/services/employeeService";
import { FaPlus, FaUserCheck, FaUserTimes, FaUsers } from "react-icons/fa";
import {
  FiCheckCircle, FiAlertTriangle, FiX,
  FiEdit3, FiUploadCloud, FiArrowRight, FiChevronDown,
} from "react-icons/fi";
...
function BulkMatriculeModal({ employees, onClose, onSuccess }: {
  employees: Employee[]; onClose: () => void; onSuccess: () => void;
}) {
...
export default function InterneEmployeesPage() {
  ...
  const handleImport = async (file: File) => {
    ...
    try {
      ...
      if (summary.skipped > 0 && summary.created === 0 && summary.updated === 0) toast.error(`Import terminé — ${summary.skipped} ligne(s) ignorée(s)`);
      else toast.success(`Import terminé — ${summary.created} créé(s), ${summary.updated} mis à jour`);
    } catch (err: any) {
    ...
    <AppLayout>
      ...
      >
        ...
        <div className="flex flex-col md:flex-row justify-between gap-3 md:items-center shrink-0">
          <div>
            <h1 className="text-3xl font-bold text-camublue-900">Gestion des employés internes</h1>
            {profileFilter !== "ALL" && (
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={handleCreate} className="bg-camublue-900 text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-camublue-800 transition">
