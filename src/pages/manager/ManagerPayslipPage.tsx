...
async function downloadBulletin(matricule: string, year: number, month: number) {
  const res = await api.get(`/api/employees/${matricule}/download-bulletin/`, {
    params: { year, month },
    responseType: "blob",
  });
...
export default function ManagerPayslipPage() {
  ...
  const refresh = useCallback(() => {
    ...
    fetchAvailableBulletins(matricule)
      .then(data => {
        setBulletins(data);
        if (data.length > 0) setOpenYear(Math.max(...data.map(b => b.year)));
      })
      .catch(() => setBulletins([]))
    ...
    <ManagerLayout>
      <div className="max-w-3xl mx-auto px-4 md:px-0">
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
          ...
          <p className="text-gray-500 text-sm mt-0.5">Téléchargez vos bulletins de salaire</p>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                      ...
                      {months.map(b => {
                        ...
                        return (
                          <div key={key} className="flex items-center justify-between bg-gray-50 rounded-xl px-4 py-3 border border-gray-100">
                            ...
                            <button onClick={() => handleDownload(b.year, b.month)} disabled={isLoading} className="flex items-center gap-1.5 text-sm text-[#003c71] hover:bg-[#003c71]/10 px-3 py-1.5 rounded-lg transition disabled:opacity-50">
