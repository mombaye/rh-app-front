...
export default function ManagerDashboardPage() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      if (isManagerMode) {
        ...
        // Mode Employé : on ne charge que les données personnelles
        const [bal, ownReqs] = await Promise.all([
          leaveBalanceService.getByEmployee(employeeId, year),
          leaveRequestService.getByEmployee(employeeId),
        ]);
        setBalances(bal);
  ...
  const stats = allStats.filter(s => !s.managerOnly || isManagerMode);
    ...
    <ManagerLayout>
      <div className="max-w-5xl mx-auto px-4 md:px-0 pb-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {/* ── Approbations en attente (Manager uniquement) ── */}
          {isManagerMode && (
