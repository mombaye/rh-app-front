...
// ─── Shift Replacements (remplacements explicites A → B) ─────────────────────

export interface ShiftReplacementRecord {
  id: number;
  date: string;
  shift_type: string;
  original_employee_id: number;
  original_employee_name: string;
  original_employee_matricule: string;
  replacement_employee_id: number | null;
  replacement_employee_name: string;
  replacement_employee_matricule: string;
  note: string;
  created_at: string;
  created_by: string | null;
}

export async function getShiftReplacements(
  date: string,
  shift_type?: string,
): Promise<ShiftReplacementRecord[]> {
  const params: Record<string, string> = { date };
  if (shift_type) params.shift_type = shift_type;
  const { data } = await api.get("/api/attendance/shift-replacements/", { params });
  return data;
}

export async function createShiftReplacement(payload: {
  date: string;
  shift_type: string;
  original_employee_id: number;
  original_employee_name: string;
  ...
  const { data } = await api.post("/api/attendance/shift-replacements/", payload);
  return data;
}

export async function deleteShiftReplacement(id: number): Promise<void> {
  await api.delete(`/api/attendance/shift-replacements/${id}/`);
}
